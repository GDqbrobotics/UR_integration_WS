...
def generate_places(x_center, y_center):
    ...
    # Poses for external circle placing
    for alpha in range(-8,8):
        x = x_center - math.cos(alpha*delta_alpha)*0.027
        y = y_center - math.sin(alpha*delta_alpha)*0.027
        angles = rotation.get_rotation_for_ur(3.14,0.15,alpha*delta_alpha)
        p = (x,y,angles)
        points.append(p)
    
    # Poses for internal circle placing
    for beta in range(-4,4):
        x = x_center - math.cos(beta*delta_beta)*0.012
        y = y_center - math.sin(beta*delta_beta)*0.012
        angles = rotation.get_rotation_for_ur(3.14,0.15,beta*delta_beta)
        p = (x,y,angles)
        points.append(p)

    # Last placing Pose at the center of the jar
    angles = rotation.get_rotation_for_ur(3.14,0.15,0)
    p = (x_center,y_center,angles)
    points.append(p)
